...
#  reference link : https://www.cnblogs.com/wuditju/p/5992129.html
    ...
    def insert_fixup(self, node):
        ...
        while node.p and node.p.color == RED:
            ...
            if node.p == node.p.p.left:
                ...
                return
            elif node.p == node.p.p.right: # 父节点是友节点
                if node_uncle and node_uncle.color == RED: # 叔叔节点是右红节点
                    node.p.color = BLACK
                    node_uncle.color = BLACK
                    node.p.p.color = RED
                    node = node.p.p
                    continue
                elif node == node.p.left: # 插入节点是左节点
